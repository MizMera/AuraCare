...
// ── SessionDetail — expanded row showing real DB residents ──
function SessionDetail({ session }) {
  const STATUS = {
    isole:     { label: '🔴 Isolated',  bg: '#FEE2E2', color: '#B91C1C', border: '#FECACA' },
    vigilance: { label: '🟡 Vigilance', bg: '#FEF3C7', color: '#B45309', border: '#FDE68A' },
    actif:     { label: '🟢 Active',    bg: '#D1FAE5', color: '#065F46', border: '#A7F3D0' },
  };
  ...
  // Use events from API (each event has a .resident object with real DB data)
  const events = session.events || [];

  // Group by resident id (or track_id if no resident linked)
  const byKey = {};
  events.forEach(ev => {
    const key = ev.resident ? `res_${ev.resident.id}` : `track_${ev.track_id}`;
    if (!byKey[key]) {
      byKey[key] = {
        key,
        resident: ev.resident || null,
        track_id: ev.track_id,
        actif: 0, vigilance: 0, isole: 0,
        confidences: [],
        timestamps: [],
      };
    }
    if (ev.event_type === 'isole')          byKey[key].isole++;
    else if (ev.event_type === 'vigilance') byKey[key].vigilance++;
    else                                    byKey[key].actif++;
    byKey[key].confidences.push(ev.confidence);
    byKey[key].timestamps.push(ev.timestamp_seconds);
  });

  const cards = Object.values(byKey).map(entry => {
    const total = entry.actif + entry.vigilance + entry.isole || 1;
    const dominant = entry.isole >= entry.vigilance && entry.isole >= entry.actif
      ? 'isole' : entry.vigilance >= entry.actif ? 'vigilance' : 'actif';
    const avgConf = entry.confidences.length
    ...
    return {
      ...
      dominant,
      avgConf,
      actif_pct:   Math.round(entry.actif    / total * 100),
      vig_pct:     Math.round(entry.vigilance / total * 100),
      iso_pct:     Math.round(entry.isole     / total * 100),
      first_seen:  entry.timestamps.length ? Math.min(...entry.timestamps) : 0,
    };
  });

  return (
    <div style={{ padding: '16px 20px 20px', borderTop: '2px solid #E9F1F6' }}>

      {/* Session summary bar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 16,
        padding: '10px 14px', background: 'white', borderRadius: 10,
        border: '1px solid #E9F1F6', flexWrap: 'wrap' }}>
        <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--midnight-green)' }}>
          📹 {session.filename}
        </span>
        ...
        <span style={{ fontSize: 12, color: 'var(--text-light)' }}>
          {session.persons_detected} person{session.persons_detected !== 1 ? 's' : ''} detected
        </span>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 14, fontWeight: 800,
          color: session.isolation_score >= 65 ? '#EF4444' : session.isolation_score >= 40 ? '#F59E0B' : '#44A6B5' }}>
          Isolation score: {Math.round(session.isolation_score)}%
        </span>
      </div>

      {/* Section title */}
      <div style={{ marginBottom: 10, fontSize: 12, fontWeight: 700,
        color: 'var(--midnight-green)', letterSpacing: '.05em', textTransform: 'uppercase' }}>
        Detected Residents
      </div>

      {cards.length === 0 ? (
        <p style={{ color: 'var(--text-light)', fontSize: 13, fontStyle: 'italic', margin: 0 }}>
          No resident events recorded for this session.
        </p>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12 }}>
          {cards.map(card => {
            const st = STATUS[card.dominant] || STATUS.actif;
            const riskCol = card.resident ? (RISK_COLORS[card.resident.risk_level] || RISK_COLORS.LOW) : null;
            return (
              <div key={card.key} style={{
                background: st.bg, borderRadius: 12,
                border: `1px solid ${st.border}`, padding: '14px 16px',
              }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between',
                  ...
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 14, color: 'var(--midnight-green)', marginBottom: 2 }}>
                      {card.resident ? card.resident.name : `Track ${card.track_id}`}
                    </div>
                    {card.resident && (
                      <div style={{ fontSize: 11, color: 'var(--text-light)' }}>
                        Room {card.resident.room_number} · Age {card.resident.age}
                      </div>
                    )}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                    <span style={{ padding: '2px 9px', borderRadius: 99, fontSize: 11,
                      fontWeight: 700, background: 'white', color: st.color }}>
                      {st.label}
                    </span>
                    {card.resident && riskCol && (
                      <span style={{ padding: '1px 7px', borderRadius: 99, fontSize: 10,
                        fontWeight: 600, background: riskCol.bg, color: riskCol.color }}>
                        {card.resident.risk_level} risk
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {[
                    { label: '🟢 Active',    pct: card.actif_pct, color: '#44A6B5' },
                    { label: '🟡 Vigilance', pct: card.vig_pct,   color: '#F59E0B' },
                    { label: '🔴 Isolated',  pct: card.iso_pct,   color: '#EF4444' },
                  ].map(bar => (
                    <div key={bar.label}>
                      <div style={{ display: 'flex', justifyContent: 'space-between',
                        fontSize: 10, marginBottom: 2, color: 'var(--text-light)' }}>
                        <span>{bar.label}</span>
                        <span style={{ fontWeight: 700 }}>{bar.pct}%</span>
                      </div>
                      <div style={{ height: 5, background: 'rgba(0,0,0,.1)',
                        borderRadius: 99, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: bar.pct + '%',
                ...
                {/* Footer: confidence + first seen */}
                <div style={{ display: 'flex', justifyContent: 'space-between',
                  marginTop: 10, fontSize: 10, color: st.color, fontWeight: 600 }}>
                  <span>Confidence: {card.avgConf}%</span>
                  <span>t = {card.first_seen.toFixed(1)}s</span>
                </div>
              </div>
            );
          })}
      ...
      {tab === 'dashboard' && (
        <>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'1.5rem', marginBottom:'2rem' }}>
            <KpiCard label="Alerts today" value={kpi.alerts_today ?? 0}
            ...
            <KpiCard label="Videos analysed" value={kpi.total_analysed ?? 0}
              color="var(--moonstone)" icon={<CheckCircle size={20}/>} sub="All time" />
                ...
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                  <thead>
                    <tr style={{ background:'#F8FAFC' }}>
                      {['File','Date','🟢 Active','🟡 Vigilance','🔴 Isolated','Score',''].map(h => (
                      ...
                      <React.Fragment key={s.id}>
                        <tr style={{ borderTop:'1px solid #F1F5F9' }}>
                          ...
                          <td style={{ padding:'12px 16px', color:'var(--text-light)', fontSize:12, whiteSpace:'nowrap' }}>
                            {new Date(s.uploaded_at).toLocaleString('en-GB')}
                          </td>
                          ...
                          <td style={{ padding:'12px 16px' }}>
                            <button
                              ...
                              style={{ padding:'4px 12px', borderRadius:8, border:'1px solid var(--moonstone)',
                                background: expandedSession === s.id ? 'var(--midnight-green)' : 'white',
                                color: expandedSession === s.id ? 'white' : 'var(--moonstone)',
                                fontWeight:700, fontSize:11, cursor:'pointer' }}>
                              {expandedSession === s.id ? 'Close' : 'Details'}
                            </button>
                          </td>
                        </tr>
