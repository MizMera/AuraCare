...
import { LogOut, Activity, AlertCircle, ShieldAlert, Users, HeartPulse, Brain } from 'lucide-react';
...
const navBtn = (active) => ({
  ...
  padding: '10px 15px', width: '100%', textAlign: 'left',
  border: 'none', cursor: 'pointer', fontSize: 14, background: 'none',
...
function StaffDashboard({ token, onLogout }) {
  const [activePage, setActivePage] = useState('residents');
    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'var(--alice-blue)' }}>
      <aside style={{ width: '260px', backgroundColor: 'var(--midnight-green)', color: 'white', display: 'flex', flexDirection: 'column' }}>
        ...
        <nav style={{ flex: 1, padding: '1rem' }}>
          <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', listStyle: 'none', padding: 0 }}>
            <li><button onClick={() => setActivePage('residents')} style={navBtn(activePage === 'residents')}><Users size={18} /> Assigned Residents</button></li>
            <li><button onClick={() => setActivePage('incidents')} style={navBtn(activePage === 'incidents')}><ShieldAlert size={18} /> Facility Incidents</button></li>
      ...
      <main style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {activePage === 'incidents' && (
          <div style={{ padding: '3rem' }}>
            <header style={{ marginBottom: '3rem' }}>
              <h1 style={{ color: 'var(--midnight-green)', margin: 0 }}>Facility Incidents</h1>
              <p style={{ color: 'var(--text-light)', margin: 0 }}>All active and recent incidents across the facility</p>
            </header>
            <div style={{ backgroundColor: 'white', padding: '3rem', borderRadius: 'var(--border-radius)', boxShadow: 'var(--box-shadow)', textAlign: 'center' }}>
              <ShieldAlert size={48} color="var(--moonstone)" style={{ marginBottom: '1rem', opacity: 0.4 }} />
              <p style={{ color: 'var(--text-light)' }}>No incidents recorded yet. Use the filters above to search across zones and incident types.</p>
            </div>
          </div>
        )}
      </main>
          ...
          <div style={{ padding: '3rem' }}>
            ...
            {errorMsg ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {data?.recent_incidents && data.recent_incidents.length > 0 ? (
                  data.recent_incidents.map((incident, idx) => (
                    <div key={idx} style={{ padding: '1.5rem', borderLeft: `4px solid ${incident.severity === 'CRITICAL' ? '#EF4444' : '#F59E0B'}`, backgroundColor: 'white', borderRadius: '0 var(--border-radius-sm) var(--border-radius-sm) 0', boxShadow: 'var(--box-shadow)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                        <strong style={{ color: 'var(--midnight-green)' }}>{incident.type_display}</strong>
                        <span style={{ fontSize: '0.8rem', color: 'var(--text-light)' }}>{new Date(incident.timestamp).toLocaleDateString()}</span>
                      </div>
...
// MAIN DASHBOARD ROUTER — unchanged
