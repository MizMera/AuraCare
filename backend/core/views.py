...
def _auto_events(sess, fi, fv, dur):
    """Assign real Resident objects to isolation events."""
    all_residents = list(Resident.objects.values('id', 'name', 'room_number', 'risk_level'))
    if not all_residents:
        return

    # Shuffle so each session picks different residents
    _rnd.shuffle(all_residents)

    # Pick how many residents to involve (at least 1, at most all)
    n_iso = min(fi, 4, len(all_residents))
    n_vig = min(fv, 3, max(0, len(all_residents) - n_iso))

    pool = all_residents[:]
    iso_residents = pool[:n_iso]
    vig_residents = pool[n_iso:n_iso + n_vig]

    for i, res in enumerate(iso_residents):
    ...
    for i, res in enumerate(vig_residents):
        IsolationEvent.objects.create(
            session=sess,
            track_id=f'ID{n_iso + i + 1}',
            resident_id=res['id'],
...
def _get_events_with_residents(session):
    """Return events enriched with real resident data from DB."""
    events = session.events.select_related('resident').order_by('timestamp_seconds')
    result = []
    for ev in events:
        entry = {
            'id': ev.id,
        ...
        if ev.resident:
            entry['resident'] = {
                'id': ev.resident.id,
                'name': ev.resident.name,
                'age': ev.resident.age,
                'room_number': ev.resident.room_number,
                'risk_level': ev.resident.risk_level,
            }
        else:
            entry['resident'] = None
        result.append(entry)
...
def _session_dict(session):
    return {
        ...
        'events': _get_events_with_residents(session),
    }
...
class IsolationSessionDetailView(views.APIView):
    ...
    def get(self, request, pk):
        ...
        data['events'] = _get_events_with_residents(session)
