...
class GaitObservation(models.Model):
    ...
    resident         = models.ForeignKey(
        ...
        related_name='gait_observations', null=True, blank=True
    )
    ...
    upload_to='gait_snapshots/',
    null=True,
    blank=True
)
    def __str__(self):
        name = self.resident.name if self.resident else 'Unknown'
        return f'{name} — {self.label} ({self.confidence:.0f}%) @ {self.recorded_at}'