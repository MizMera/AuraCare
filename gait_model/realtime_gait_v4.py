...
sys.path.insert(0, r'C:\Users\yomna\Downloads\AuraCare\AuraCare-main\AuraCare-main\backend')
...
def save_clip_and_send(patient_name, label, confidence, features, frames, fps, width, height, zone='East Wing Corridor'):
    """Save a snapshot and send to backend."""
    try:
        if frames:
            # Take the middle frame as snapshot
            mid_frame = frames[len(frames) // 2]
            tmp = tempfile.NamedTemporaryFile(suffix='.jpg', delete=False)
            tmp_path = tmp.name
            tmp.close()
            cv2.imwrite(tmp_path, mid_frame)
            ...
            send_to_backend(patient_name, label, confidence, features, zone)
    except Exception as e:
        print(f'  ⚠️  Could not send snapshot: {e}')
                    ...
                    if feats:
                        ...
                        if tr['patient_id'] != 'unknown':
                            log_session(tr['patient_id'], tr['patient_name'],
                                        feats, tr['label'], tr['confidence'])
                            print(f'  💾 Logged: {tr["patient_name"]} — {tr["label"]}')
                tr['frame_buffer'].append(frame.copy())
                if len(tr['frame_buffer']) > CLIP_DURATION_FRAMES:
                    tr['frame_buffer'].pop(0)
